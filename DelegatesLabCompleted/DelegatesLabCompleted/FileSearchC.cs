...
namespace DelegatesLabCompleted
{
  class FileSearchC
  {
    ...
    public event BeforeFileFoundHandler BeforeFileFound;
    public event FileFoundEventHandler FileFound;

    List<FileInfo> files = new List<FileInfo>();

    public FileSearchC(String lookIn, String fileSpec, Boolean searchSubFolders)
    {
    ...
    private void HandleFiles(FileInfo[] localFiles)
    {
      ...
      foreach (FileInfo file in localFiles)
      {
        ...
        {
          BeforeFileFound(file, ref cancel);
          if (!cancel)
          {
            OnFileFound(file);
          }
